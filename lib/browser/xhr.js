...
//                input = new IO(inputStream, null);
            ...
            var buf = Packages.java.lang.reflect.Array.newInstance(Packages.java.lang.Character.TYPE, 1024);
            var responseArray = new ByteArray();
            var bytesRead = 0;
            if (inputStreamReader !== null) {
                while ((bytesRead = inputStreamReader.read(buf, 0, 1024)) != -1) {
                    for (var i=0;i<bytesRead;++i)
	                    responseArray.push(buf[i]);
                    this.responseRaw = responseArray.toByteString();
                    this.readyState = LOADING;
    ...
    this.responseXML = null;
    if (this.responseText && (!HashP.includes(this._responseHeaders, "Content-Type") || HashP.get(this._responseHeaders, "Content-Type").match(/((^text\/xml$)|(^application\/xml$)|(\+xml$))/)))
        try { this.responseXML = parser.parseFromString(this.responseText); } catch (e) {}
